...
const HARD = [
  /(^|[^A-Za-z0-9_])(M[0-9]+|(issue-tree|logic-tree|sequence)-m[0-9]+)(?![A-Za-z0-9_]|[ ,]-?[0-9.])|(^|[^A-Za-z0-9_\/-])m[0-9]+(?![A-Za-z0-9_])|(^|[^A-Za-z0-9_])Task [0-9]+/,
  /レビュー(指摘|で|の|:|：|\)|）)|依頼者|申し送り|フォローアップ|残件から|踏んだ罠|で確定|で決定|実機修正|指摘\s*[0-9]|人間の(裁定|判断)/,
]
// 現在形の文にも出うる語。列挙だけして、残すかは読んで決める
const SOFT = /当初|以前|かつて|元々|もともと|変更前|実機確認|→ ?[0-9]+|に伸ばし|から分離|へ引き上げ|へ下が|へ移し|に移した|一本化した|20[0-9]{2}-[0-9]{2}-[0-9]{2}/
