 ...
 *——結果として、この面の上に載る子を守る門番は上の3行しかない。
 *
 * **いま実際に載るものは3行で尽きている。** 解決を掲げた箱の中身は
 * タイトル（`ink`）・理由とシェブロン（`ink-muted`）・箱の枠（`rule`）と、
 * **旗のバッジ自身**（`judge-yes` の濃い面）である。**仮説の行のバッジは
 * `faint`**（枠と文字が `ink-faint`、面を持たない）——旗を掲げた課題の行は
 * 自分の旗で抑制されるため（`derive.ts` の `suppressedIssueIds` は自己包含）。
 ...
 *
 * **測れていないものは2つあり、どちらも承知のうえである:**
 *
 * 1. **`ink-faint`（抑制された仮説行の点・文言・バッジ）。** 表に入れていない
 *    のは、`ink-faint` が `BACKGROUNDS` 3面に対して既に 3:1 を課されており、
 *    ここへ足すと**ダークで MARGIN（1.03）を割る**ため（現行値の実測は
 *    ライト 3.77:1・ダーク **3.05:1**。3:1 は満たすが 3.09 に届かない）。
 *    **閾値ちょうどを置かないのがこのファイルの流儀**（`MARGIN` の註）なので、
 *    門番にするなら面の L を一段下げること（ダーク 0.265 で 3.09、0.26 で 3.14。
 *    `ink` / `ink-muted` / `rule` はいずれも余裕が増える方向にしか動かない）。
 *    **忘れられた要件ではなく、値を動かす判断を保留しているだけである。**
 * 2. **旗のバッジ（`judge-yes` の濃い面）とこの面の分離。** 面どうしなので
 *    原理的にこの表では測れず、`FACE_PAIRS` も「白黒で判別する」判断軸の
 *    2面のためのものである。実測はライト **1.26:1**・ダーク 8.18:1——
 *    **ライトでは緑のバッジが緑の箱にほぼ溶ける。** バッジの中の文字は
 *    `judge-yes-fg` が `judge-yes` に対して 4.5:1 を課されているので読めるが、
 *    **チップの輪郭は見えない**（`border-transparent` なので枠も無い）。
 *    ライトの実機で「バッジが箱に埋もれる」なら、バッジ側に枠を与えるか、
 *    面の L を下げること。**この見え方は機械では拾えない。**
 */
