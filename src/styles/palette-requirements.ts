 ...
 * 2.996:1）。検査は `req.min`（3.0）で見ており、**`MARGIN` はテストの閾値では
 * ない**——`MARGIN` は「閾値ちょうどを置かない」ための**値を選ぶ側の規律**で、
 * 実際に読むのは `palette-fit.mjs` の提案（`fitLightness` に渡す条件）だけである
 *（`MARGIN` の註がそう書いている）。だからダークを **0.27（3.045:1）に戻しても
 * 検査は緑のまま通る。** 0.26 まで下げたのは人がその規律を当てた結果であって、
 * 機械に強制されたのではない。**この2段を混同しないこと**——「テストが通る」は
 * 「余裕がある」を意味しない。
 *
