...
- **ステータスというフィールドは存在しない。** 仮説の現在の判断は `events`（追記専用の配列）の**最後の要素**の `kind` から読む。`events` が空＝まだ決めていない（未決）。`supported`＝検証して支持／`rejected`＝検証して棄却／`supportedWithoutTest`＝自明に成立／`rejectedWithoutTest`＝検証せず棄却／`onHold`＝保留（見たが判断できなかった）／`deferred`＝今回見送り／`deferredToMainDev`＝本開発送り
