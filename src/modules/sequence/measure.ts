 ...
 * 折り返しの規則そのものは `@/core/canvas/wrap.ts` が持つ。
 */
 ...
 * （`pl-18` = 4.5rem = 72px）。接頭「考慮不要」の本文実幅は 4字×14px＝56px
 * で、`left-2`(8px) から始まっても 64px（8+56）しか要らないので、72px には
 * 8px の余裕がある。**72px は動かさない**——余る 8px は接頭と本文の間の
 * 空きになるだけで、狭すぎて重なる向きの誤りではない。
