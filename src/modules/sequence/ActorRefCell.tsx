 ...
 * 選ぶ方式は採らない: 補完の出番が少なく、その場でアクターを作る必要も無い。
 *
 * アクターの追加は、ヘッダの `Enter` とツールバーの「アクターを追加」の2本である。
 *
 ...
 * 欠落の面（破線＋淡い `missing` の面）で示す（決定1）。
 * 「（未定義）」とは書かない——データに無い文字列を画面が作り出す
 * ことになり、コピーすれば仕様のように読めてしまう。押す場所が見えない
  ...
  return (
    <DropdownMenu open={props.open} onOpenChange={props.onOpenChange}>
      <DropdownMenuTrigger
        ...
        //
        // **値は 26px で固定。導出しない。** 本文は 14px・`--tw-leading: 1.2` で
        // 内容高が 14×1.2=16.8 ＋ `py-0.5` 4 ＋ 枠 2 ＝ 22.8px にしかならないので、
        // 26px の下限が常に勝ち、空名でも文字入りでも実高は 26px で揃う。
        //
        // 一方 `StepShapeCell`（種別セル）は `min-h` を持たず 22.8px で描かれる
        // ので、箱の高さは 3.2px 違う。**これは意図の範囲である**——両者は
        // 親が `top: railTop` の絶対配置で置いており**上端が揃う**ので、
        // 下端の 3.2px 差は行の段差として読めない。揃えたくなったら
