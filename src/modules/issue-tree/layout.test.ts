...
describe('layoutIssueTree', () => {
  ...
  /**
   * 帯の「FB待ち」と行の表示を一対一にする。**同じテストで有り／無しの両方を見る**
  ...
  /**
   * 仮説を持たない課題の箱をタイトルの自然幅にすると、箱幅が内容で決まり、
   * 同じ列の中でバッジの右端が散って、「どれが未決か」を知るのに全ノードを
   * 読む必要が出る
   *（rev 9章 D3 rev.3 ＝ スキャン性）
   */
  ...
  it('同じ深さの箱では、旗のバッジの右端が揃う', () => {
    ...
    // 幅を内容から導出する実装に戻しても見送りバッジの幅を含めて BOX_WIDTH に
    // 落ちるため、区別が付かない。c は「仮説も見送りも無い箱はタイトルの
    // 自然幅」に戻すと、短いタイトルぶんだけ箱が狭くなる側——戻すと c.rect の
    // 幅が BOX_WIDTH を割り、下のアサーションが赤くなる
  ...
  /**
   * **箱の幅は 360 である。** 旗のトグルが2つ（見送り／解決）あり、タイトルの
   * 右に空ける枠が広い分を箱の側で吸収する（タイトルは 224px 前後）。
   *
   * **ここだけは `BOX_WIDTH` を使わずに数字を書く。** 上の「トグル2つぶんの枠」の
   * テストは `BOX_WIDTH` から期待値を組むので、定数を戻しても緑のまま通る
   *——**戻したことに気づく番人がどこにも無い**。伸ばした事実そのものはここが持つ。
   *
  ...
  it('畳んだ箱の幅は 360 で、タイトルはそのぶん広い 224px を取る', () => {
    ...
    for (const [i, box] of out.issues.entries()) {
      ...
      // **開閉トグル（シェブロン）が無いので、タイトルの左に空けるはずの
      // 20px が無い**
  ...
  /**
   * **タイトルは内容の左端から始まる。** 開閉トグル（シェブロン）ぶんの
   * 20px を空けたままにしないこと——**片方だけ残ると、誰も使わない
   ...
   * 行が1本も無い 780 幅の箱が残らないようにする番人。
   *
   * **選択そのものは残る**——選ばれた印（`selected`）と
