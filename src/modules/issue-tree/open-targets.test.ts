...
describe('listOpenTargets（要対応の並び）', () => {
  it('課題の DFS 順に、課題の「仮説なし」→ その仮説の問い、の順で並ぶ', () => {
    ...
    // 取り違えを検知する（種類で舐めると 未決→保留→未判断＝A, B, C の順になる）
    ...
    const hypotheses = [
      hypothesis(2, id(2), { events: [{ kind: 'onHold', note: '' }] }),
      hypothesis(1, id(2)),
      // 未判断だけを立てるので、イベントを付けたうえで最新を onHold 以外にする
      //（0件だと未決も一緒に立ち、この仮説から2件出る）
      hypothesis(3, id(2), { events: [{ kind: 'supported', note: '' }], pendingNotes: ['締め忘れ'] }),
    ]
  ...
  it('1つの仮説に複数の問いが立つときは 未決→保留→未判断 の順（tallyLine の内訳と同じ並び）', () => {
    const issues: IssueNode[] = [{ id: id(0), parentId: null, text: '根', events: [] }]
    // 保留のまま FB が残っている＝保留と未判断が同時に立つ
    const hypotheses = [
      hypothesis(1, id(0), { events: [{ kind: 'onHold', note: '' }], pendingNotes: ['FB'] }),
    ]
    expect(targetsOf(issues, hypotheses)).toEqual([
      ['hold', { cell: 'hypothesis', index: 0 }],
      ['judgement', { cell: 'hypothesis', index: 0 }],
    ])
  })
  ...
  it('抑制された配下は列に入らない（posed が立てていない）', () => {
    const issues: IssueNode[] = [
      ...
      { id: id(1), parentId: id(0), text: '見送り', events: [{ kind: 'deferred', note: '' }] },
      { id: id(2), parentId: id(1), text: '配下の葉', events: [] },
    ]
    const hypotheses = [hypothesis(1, id(1)), hypothesis(2, id(2), { pendingNotes: ['FB'] })]
    expect(targetsOf(issues, hypotheses)).toEqual([])
  })
