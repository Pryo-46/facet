...
export function listOpenTargets(
  ...
  data.issues.forEach((node, index) => {
    ...
    for (const hi of rowsOf.get(node.id) ?? []) {
      ...
      // ——仮説につき1つしか出せない形にすると、**「FB待ち 2」と言いながら
      // 2回で一巡しない**破れが起きる。`ask` のセル（`cell-keys.ts`）が
      // あるので問いごとに出せる。
      //
 ...
 *（`askIndex`）まで見る。**
 *
 * `cell` と `index` だけでは、問いの行き先を同じ物差しで測ると、**同じ仮説の
