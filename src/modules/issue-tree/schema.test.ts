...
describe('issueTree のスキーマ検証（レベル1）', () => {
  ...
  /**
   * **v4 で反転した。** v3 までは `maxItems: 1` を入れず「排他はスキーマの担当
   * ではない」としていたが、**アプリは最新1件しか見せず最新1件しか書き換えない**
   * ので、2件目以降は「ファイルにあるのに画面に出ないデータ」になっていた。
   * 設計ノート D2 が当初からこの列を「0 件か 1 件」と定めているとおりに、
   * スキーマの側を揃えた。
   *
