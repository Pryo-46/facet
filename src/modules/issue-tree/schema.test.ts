...
describe('issueTree のスキーマ検証（レベル1）', () => {
  ...
  /**
   * **アプリは最新1件しか見せず最新1件しか書き換えない**ので、2件目以降は
   * 「ファイルにあるのに画面に出ないデータ」になる——設計ノート D2 はこの列を
   * 「0 件か 1 件」と定めている。仮説の `events` も v4 で同じ形に絞ってあるので、
   * 課題側だけ緩いままなのは非対称になる。
   *
   * 手書きの2件以上のファイルが版だけ上がって開けなくなることは、意図した
   * 結果である（考慮しない）。もう一つの制約——`commands.test.ts` が守る
   * 「戻すと最新の1件だけが消える」という前提——は、型がタプル
   * （`[] | [IssueEvent]`）になっているので、2件以上の入力はそもそも作れない
   *（`commands.test.ts` の該当 `it` も同じことを固定している）。
   *
   ...
   *（間違えて付けた判断を消せないことのほうが高くつく）。**判断が2件並ぶ
   * v3 のファイルは、この一行で開けなくなる。**
   *
