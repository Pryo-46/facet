...
/**
 * 課題の箱1つ。**箱は課題だけであり、仮説はこの箱の中の行になる**。
 *
 ...
 *    `ink-faint` に落とす——白い面（`bg-surface`）のままだと見送りの枝が
 *    「まだ生きている枝」に見える。**`bg-canvas`（地の色）には落とさない**——箱が
 *    背景に溶けて木の形が読めない。**`opacity-*` で薄くしない**（検算した
 *    コントラストを割る。`ink-faint` は `BACKGROUNDS` 3面——`surface-muted` を
 *    含む——への 3:1 が `palette.test.ts` で機械検査済み）
 * 3. **自分自身が旗（見送り／解決）を掲げている箱は面を持つ**
 *    （`docs/issue-tree/仮説検証モジュール-設計ノート.md` D8）。未決とは違って旗は稀で意図的な判断なので、1の理由
 *    （未決を面で塗ると図が警告で埋まる）はここには効かない。**配下も同じ面を
 *    持つ**ので、面が運ぶのは「表明の所在」ではなく**凍結の範囲**になる——
 *    誰が掲げたかは文字の濃さ（当人＝`text-ink`／配下＝`ink-faint`）とバッジ
...
export function IssueBox(props: IssueBoxProps) {
  ...
  // 薄いまま」で見ている）。**解決の面（`judge-yes-face`）があるぶん賭け金は
  // 高い**——逆にすると、凍結された枝の途中に淡い緑の箱が1つだけ
  // 灯り、「その1件はまだ考える」と読めてしまう。順番は動かさないこと。
  //
  // 旗を掲げた箱は面を持つ。**見送りと解決は面を分ける**——理由は
  // 「一目で『解決方針が決まった課題』＝これ以上考えなくてよい とわかる」こと
  // （設計ノート D8）。見送りは一段沈んだ面（`surface-muted`）のまま、**解決だけ淡い緑
   ...
   *
   * **面と枠のクラスは片方だけ出す**（上の `face` と同じ約束）ので、
   * ここは1本の三項で枠を1つだけ選ぶ
   */
   ...
   * **`[data-panel]` はここでは効かない**——ポータルはパネルの DOM の外にある。
   *
   * **下の選択子の `[role="menuitem"]` と `[role="menu"]` は、片方を消しても
   * どのテストも赤くならない。冗長だと判断して消さないこと。** 項目は面の
   * 子なので、片方を落としても `closest` がもう片方で止まる——**番人が
   * 効くのは両方を落としたときだけ**である。片方だけ残った状態は
   * 「いまは無害だが、Radix が面と項目の入れ子を変えた日に静かに破れる」。
   * 両方を消す変異で赤くなるのは `IssueTreeEditor.dom.test.tsx` の
