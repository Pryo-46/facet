...
export interface IssuePlacement {
   ...
   * 展開の鍵は課題を指すので、仮説が消えても鍵は残ったままである——**行が
  ...
  /**
   * 選択された課題ノードの**末尾**に置く「＋ 仮説を追加」の帯。
   * 選択されていなければ null。
   *
   * **`expanded` ではなく `selected` で出る**——仮説を
...
export interface HypothesisPanel {
   ...
   *
   * **`detail` はここには出ない**——「どう作るか」は独立した節で、
   * 価値仮説の次に座る（下の `detail`）。**ゴミ箱（仮説の削除）は
   ...
   *
   * 独立した節にする理由は、ソリューション仮説のタイトルの下に本文として
   * 描くと（スキーマが `detail` を「ソリューション仮説の**詳細**（どう作るか）」
   * と説明している通りの入れ子にすると）、**タイトルと詳細のどちらが仮説の
   * 本体か**が見分けられなくなるため。節に上げて見出しを与え、**価値仮説
   * （なぜ効くか）の次**——「何を・なぜ・どう」の順——に置く
   */
...
/**
 * **`JUDGEMENT_TRIGGER_LABELS`（「判断を追加」「判断を変える」）は無い。**
 * トリガーは状態のバッジ自身なので、**トリガーに固有の文言は
 * 存在しない**——描くのは `EVENT_KIND_LABELS` ／ `BADGE_LABELS` の語であり、
 * 何をするボタンかはアクセシブル名（`仮説{N}に判断を追加`）が運ぶ。
 * 帯の高さを決める `judgeLabelH` も**バッジの高さで測っている**
 *（下の「検証結果の節」を見ること）
 */
 ...
 *
 * **「どう作るか」の節だけ、見出しと促しが入れ替わっている**——見出しに採った
 * のは説明の括弧の中（＝促しの側）で、**残る半分の「詳細」がプレースホルダに
 * 来る**。ここを「どう作るか」のままにすると**同じ語が見出しの真下に2つ並ぶ**。
 * `詳細` は欄の呼び名としても既に使われている
 ...
 *
 * **キャンバスの逐語は「聞きたいことを追加」**——画面では「SHに」を
 * 前に付ける（`ASK_LABEL` の解説）。
 *
...
export function layoutIssueTree(
  ...
  const plans: RowPlan[] = data.hypotheses.map((h, hi) => {
    ...
    if (!open) {
      ...
      /**
       * 「FB待ち」の行バッジ。帯の集計（`tallyQuestions`）と行の表示を一対一に
       * する——数だけが増えて、どの行かが図から読めないのを防ぐ。
       *
     ...
     * その下に**パネルの全幅**で座る。
     *
     * **`ACTION_HEIGHT`（24px）は入れない。** 判断のトリガーは「判断を追加」
     * という文言のボタンではなく**バッジ自身**（`BADGE_HEIGHT`）である。帯に
     * ボタンぶんの 24px を入れると、**誰も使わない 2px が根拠の欄を押し下げる**
     */
  ...
  const boxes: BoxPlan[] = data.issues.map((node, i) => {
    ...
    // 幅が固定なので、枠が文章を食って下限を割る経路は無い——枠は常に
    // 固定幅の中から取られる。
    //
    ...
    //
    // **展開している課題だけが `EXPANDED_BOX_WIDTH`。** これも固定値であって
    // 内容から導出しない——導出にすると、開いた瞬間に幅が文言の長さで変わる
    const width = open ? EXPANDED_BOX_WIDTH : BOX_WIDTH
    const contentWidth = width - ISSUE_INSET_X * 2
    /**
     * **タイトルは内容の左端から始まる。** 開閉は箱のクリックによる選択なので、
     * 開閉トグル（シェブロン）のぶん（`CHEVRON_SIZE + CHEVRON_GAP` ＝ 20px）の
     * 空きは無い。タイトルはそのぶん広い
     */
    ...
    /**
     * **選ばれた課題ノードの末尾には「＋ 仮説を追加」が座る。** 仮説を
     * 足す動線はキーには無くマウスにしかない。**その高さを箱に足すこと**
