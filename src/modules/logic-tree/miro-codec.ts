 ...
 *
 * 器と木を分けてあるのは、**器の不具合が木のテストでは見えない**ため——
 * 終端タグが欠けても復号は末尾の余分を黙って捨てるので通り、往復テストも
 * 通ってしまう。だから器のテストは**原本のバイト列と照合する**。
 *
 * 形式の詳細は docs/superpowers/plans/ の miro-clipboard-design.md
 */
