...
describe('glossaryToMarkdown: 絞り込み', () => {
  ...
  it('visible を渡さなければ従来どおり全件（既存の呼び出しが1文字も変わらない）', () => {
    // 用語を1件だけにすると、「絞り込んで空にする実装」でも `f(data)` と
    // `f(data, null)` が一致してしまい、等価性の確認にしかならない。
    // 2件用意し、両方が出力に含まれることまで確かめて初めて「全件」の名に見合う
    const data: GlossarySchemaVersion1 = {
      schemaVersion: 1 as const,
      type: 'glossary' as const,
      title: 'T',
    ...
    const md = glossaryToMarkdown(data)
    expect(md).toBe(glossaryToMarkdown(data, null))
    expect(md).toContain('受注')
    expect(md).toContain('与信')
  })
})
