...
スキーマ検証は同梱の生成物（`scripts/generated/validate.mjs`）が行う。**スキーマの差し替えはできない**——検証は生成時に焼き付いており、アプリが開けるかどうかの基準は同梱スキーマそのものだからである。`schemas/sequence.schema.json` は正規形のキー順を決めるために読まれ、facet の原本とのバイト一致はアプリのテストが強制している。
