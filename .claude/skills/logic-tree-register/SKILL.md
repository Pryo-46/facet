...
**`--out` でも `--check` でも、書き出す本文は配列を DFS 行きがけ順へ正規化してから作る。** `--check` は書き込まないが、正規化した本文と入力ファイルを比較して差分の有無を報告する。既存ファイルが既に行きがけ順なら差分は追加分だけだが、乱れた順のファイルを渡すと配列全体が動く。その場合は報告で伝える。
