...
| `judge-yes-face` | 既定値を示して確認 | **`judge-yes` と同じ色相**で、L 0.93〜0.96（ダーク 0.26〜0.30）、C 0.04〜0.06 の淡い緑。課題ツリーの「解決した課題の箱」の地（issue-tree-m5）。**上の3面と要件が1つ違う**——載るのは `ink` / `ink-muted` / **`rule`** の3つで、判断軸には線色のトークンが無く箱の枠が `rule` だからである（`FACE_REQUIREMENTS`。`rule` は文字ではなく枠なので 3:1）。**ダークはこの `rule` の 3:1 が効いて上の3面より一段暗くなる**——L 0.30 では届かない |
...
**淡い面（`*-face`）の行だけは、提案どおりに直さない。** 「面の文字」の節には `ink / missing-face`・`missing / missing-face` のような行が12本出る（4面×3色）が、そこに出る提案は**文字側の L を動かせ**という内容である。`ink` も `ink-muted` も、面に載るもう1色（3面は線色、`judge-yes-face` は `rule`）も他の面に縛られているので、動かすのは**面（`*-face`）の L の方**——ライトなら明るく、ダークなら暗くする（手順5の範囲: L 0.93〜0.96 / 0.26〜0.32）。
