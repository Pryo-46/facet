...
//
// 使い方:
//   node scripts/new-id.mjs            → error_XXXXXXXXXX を1件
//   node scripts/new-id.mjs 5          → 5件（1行1件）
//   node scripts/new-id.mjs 3 --prefix term
//
...
if (!/^[a-z]+$/.test(prefix ?? "")) {
  console.error(
    `entityPrefix は小文字英字のみ: 受け取った値 = ${JSON.stringify(prefix)}`
  );
  process.exit(2);
}
