...
問いは4つ——葉の課題に仮説が無ければ「**仮説なし**」、仮説に判断が無ければ「**未決**」、見たが決められなければ「**保留**」、文言のある問い（聞きたいこと）に FB が1件も無ければ「**FB待ち**」。**祖先の課題に旗（見送り・解決）が立っている配下では、この問いが立たなくなる**（旗は最上位に一度だけ付け、配下への波及は毎回計算する）。

画面は俯瞰に寄せてある——**箱になるのは課題だけ**で、仮説はその中の1行、判断は行末のバッジ1つ。判断・以前の判断・FB は、いま見ている仮説を展開したときだけ出る。帯に出る「要対応」の内訳は押せて、押すと次の要対応へ視点が飛ぶ。

<!-- SCREENSHOT: issue-tree-editor.png
     撮り方: 課題ツリーを開き、仮説を1つ展開した状態（展開した仮説にだけ判断・以前の判断・FB が出る）。
...
```
npm install        # 依存の導入＋スキーマからの型生成＋同梱 Skill の生成物づくり
