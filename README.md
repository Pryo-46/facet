...
- 内部処理 → 「失敗が確定したら？」
- 応答を待つ呼出 → 「失敗が確定したら？」＋「結果が不明だったら？」（さらに「実は実行済みだったら？」＝冪等性の問い）
- 投げっぱなしの呼出 → 「結果が不明だったら？」
- 応答 → 問いは立たない（応答の失敗は、対になる呼出側の「結果が不明」が受ける）
...
このリポジトリの **[`sample-project/`](sample-project/)** が、4ツールを同じ題材（中途採用の応募管理）で埋めたお手本。**わざと未決を残してある。**
...
プロジェクトフォルダを開くと、`.claude/skills/` に登録用 Skill が3本置かれる（`glossary-term-register` / `error-catalog-register` / `sequence-register`）。**会話でヒアリングしながらデータを組み立てる** ためのもので、ID の採番・スキーマ検証・正規形での書き出しは同梱スクリプトが行う。手書きの JSON が混ざらない。
