...
画面は俯瞰に寄せてある——**箱になるのは課題だけ**で、仮説はその中の1行、判断は行末のバッジ1つ。根拠・以前の判断・FB は、いま見ている仮説を展開したときだけ出る。帯に出る「要対応」の内訳は押せて、押すと次の要対応へ視点が飛ぶ。

<!-- SCREENSHOT: issue-tree-editor.png
     撮り方: 課題ツリーを開き、仮説を1つ展開した状態（展開した仮説にだけ根拠・以前の判断・FB が出る）。
