...
**2026-08-24、人間が実機で一巡し、上のチェックリスト9項目すべて問題なしと判断された。** チェックリスト自体は「空のまま写した」当時の記述であり、追記専用の原則どおり書き換えていない——この節がその結果の記録である。項目9（ガターの未回答の面が `canvas` の方眼から分離して見えるか。M21 からの持ち越し）も問題なしで、M21 の申し送りに予備策として書かれていた「ライトの `--missing-face` を L 0.95 → 0.93 へ下げる」は**発動していない**。この確認によるコード・色値の変更は無い。`open-issues.md` の「次に手を付ける候補」7件目（M22 の実機確認が未実施）はこれを受けて消した。
