...
- **トークンは 15 → 18。** `missing-face` / `invalid-face` / `pending-face` を足した。各面は同じ軸の線色と同じ色相で、その上に `ink` / `ink-muted` / その線色の3つが 4.5:1 で載る（`FACE_REQUIREMENTS` が9行ぶん課す）。**ライトの `pending-face` は当初 `oklch(0.94 0.04 250)` を置いたが sRGB の外**（往復で C 0.033）で「色域」の検査に落ちたので、L ではなく C を下げて `oklch(0.94 0.03 250)` にした
- **`DISTINCT_PAIRS` には淡い面を入れない。** 欠落・無効・着信の識別は線の色と線種が運ぶ（その6組は既に3色覚ぶん検査済み）。白へ寄せた面どうしの色差は原理的に小さく、実測でも ΔE 0.05 前後（ライトの `missing-face` / `invalid-face` が標準 0.059・D型 0.049）しか出ない。**通すために面を濃くすれば「淡い面」であること自体が壊れる**ので、面の色差に意味を負わせない判断を `palette-requirements.ts` にコメントとして残した
