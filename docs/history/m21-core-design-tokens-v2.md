...
## 実機確認の続き（2026-08-24）——セルの輪郭を外した

淡い面が入った状態で、用語集・エラーカタログのセルの**輪郭（`outline`）は要らない**と判断された。表の中では輪郭がテーブルの罫線（`border-b border-grid`）と競合し、情報ではなくノイズになる。

**決めたこと：セルは淡い面だけ**（`CELL_FACE_CLASS` から `outline-*` を外す。規約2「淡い面＋線」の例外として rev 9章に明記）。欠落と無効の区別は面の色相（黄／赤）が運ぶ。**バッジは線を残す**——線種（破線＝まだ見ていない／実線＝保留）が情報を運ぶのはバッジだけで、セルにはその区別が無い。ガターのスロット・ノード・参加者ヘッダ・`#N` は表の罫線の中に居ないので今回は触っていない。

触ったのは `src/core/list-editor/cell-face.ts` の `CELL_FACE_CLASS` と doc、rev 9章の2文。
