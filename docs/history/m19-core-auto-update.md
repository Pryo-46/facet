...
### 3. 多重起動の錠前を `setUpdateState` の更新関数の中で判定するコードを計画が書いていた

React は state 更新関数（`setState(prev => ...)`）が同期に呼ばれる保証を持たないため、あの形だと連打と起動時チェックの重なりで2本走りうる。`src/App.tsx` に `updateBusyRef`（ref。実行中かどうかの錠前そのもの）と `canCheck(state)`（`src/core/update-check.ts`。`disabled` を導く純関数）の2つに役を分けた。

### 4. mac のテストが退化していた（実装者が発見）
