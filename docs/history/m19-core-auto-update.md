...
エージェントは Tauri の GUI を操作できないため、確認はすべて人間が行った。**開発ビルドで見える3項目は完了し、実リリースを要する6項目（4〜9、および 5.5）は未実施。**

まず開発ビルドで見える範囲（`npm run tauri dev`）——**3項目とも人間の手で確認済み**:

1. ネットワークを切って起動し、何も表示されないこと（静かな失敗）— **確認済み**
2. ネットワークを切って更新ボタンを押し、トーストにエラーが出ること — **確認済み**
3. ネットワークを繋いで押し、「facet は最新版です」が出ること — **確認済み**

### 確認の過程で分かったこと: エンドポイントの 404 は「更新なし」にならない

3 を確かめようとした最初の試行で、手動チェックが `更新を確認できませんでした: Could not fetch a valid release JSON from the remote` を返した。原因は**どのリリースにも `latest.json` が存在しなかったこと**で、エンドポイントは 404 を返していた（`curl` で確認: `releases/latest/download/latest.json` → `releases/download/v1.0.0/latest.json` → 404）。

**Tauri の updater が「更新なし」と解釈するのは HTTP 204 だけで、404 はエラーである。** `releases/latest/download/` は常に最新のリリースを指すため、**将来どこかのリリースで `latest.json` を上げ忘れると、その瞬間から全利用者の更新チェックがこのエラーになる。** `docs/release.md` が「上げ忘れると更新経路が止まる」と警告している事態が、実物として再現した形になる。

正常系（「最新版です」）を v1.0.1 を待たずに確かめるため、**v1.0.0 のリリースに `version: "1.0.0"` の `latest.json` を1本追加した。** `signature` は空文字列にしてある——v1.0.0 は署名鍵を作る前にビルドしたので `.sig` が存在せず、かつこの欄が読まれるのはダウンロードした成果物を検証するときだけで、版が同じなら updater は「更新なし」で止まってここに触れないため。1.0.0 より古い版は存在しないので、これをダウンロード対象として引く経路も無い。

この結果、1 と 3 は**エンドポイントが 200 を返す状態**でも確認されている（起動時は無言、手動チェックは「facet は最新版です」）。2 は 404 が返っていた時点の観測で、経路としては同じ。

次に、中身の変更がほぼ無い v1.0.1 を本物のリリースとして出し、v1.0.0 から実際に更新されるかを見る（ローカルの静的サーバでは代用しない）。**以下6項目は未実施**:
...
残る未実施は `docs/open-issues.md` の「次に手を付ける候補」に追加した。あわせて、実機確認でしか確定しない2点（自動再起動の有無・SmartScreen の有無）を「挙動の穴」に `[M19]` タグで追加した。
...
- 「次に手を付ける候補」に2件: M19 の実機確認が未実施であること（**その後、開発ビルドで見える3項目は完了した。残るのは実リリースを要する6項目**）、未署名配布そのもの（mac の Gatekeeper・Windows の SmartScreen）が M19 では解決していないこと。
