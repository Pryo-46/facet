...
コミット範囲: `a0b309e`（スペック）〜本コミット。実装は `3d3e66f`〜`b312f08`（Task 1〜10、計13コミット）。**Task 8 はコミットを産んでいない**——旧段の残骸の全数 grep と生成 CSS 4点の確認だけで、直すものが無かった。fix round が入ったのは Task 5（1巡。エラーカタログ `occurrence` 列の分類）・Task 7（1巡。旧段を現在形で断言するコメント7行）・Task 10（1巡。rev 9章のレビュー指摘3件）。**Task 11（本コミット）は申し送りの記述だけ**で、**実機確認は人間の作業として未了のまま残す**（[`../open-issues.md`](../open-issues.md) の「次に手を付ける候補」7件目）。
