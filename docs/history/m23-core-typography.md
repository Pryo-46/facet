...
**2026-08-25、人間が実機で一巡し、チェックリスト11項目すべて問題なしと判断された。** チェックリスト自体は「空のまま写した」当時の記述であり、追記専用の原則どおり書き換えていない——この節がその結果の記録である。あわせて確認するとした `ActorRefCell` の空名トリガーと文字入りの高さも問題なしだった。この確認によるコード・値の変更は無い——**同じ一巡で出た計画外の2件（見送りトグルの幾何・グレースロットの削除ボタンの縦位置）は上の「実機確認の先行所見」の節のとおりで、チェックリストの消化とは別である**。`open-issues.md` の「次に手を付ける候補」7件目（M23 の実機確認が未実施）はこれを受けて消した。
