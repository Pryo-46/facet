...
コミット範囲: `43a4ef3`（着手前の走査で見つけた計画の欠陥3件の修正）〜 本コミット。計画は Task 1〜7 の7本。fix round が入ったのは **Task 4**（1巡・SKILL.md の文言のみ）、**Task 6**（1巡・判定器の穴2件）、**Task 7**（1巡・章参照と `docs/README.md` の欠落行）、そして**全ブランチの最終レビュー**（Important 2件・Minor 4件を1巡でまとめて処理）である。
...
| 3 | `4a5d708` | `logic-tree-write.mjs`（検証・DFS 正規化・整合性4種・集計・書き出し）＋ `src/modules/logic-tree/skill-write.smoke.test.ts` |
...
### 整合性の報告は「アプリが開くことになるファイル」の並びで行う——経路で指す先が変わる
...
したがって、**アプリが同じファイルを開いたときの指し方と揃うのは「そのファイルの並び」だけ**である。そして**どのファイルを指すかは経路で変わる**——`--check` が指すのは入力ファイルそのもの、`--out` が指すのは**これから書き出すファイル**（＝並べ替えた後）である。

| 経路 | 報告に使う並び |
| --- | --- |
| `--check <file>` | 入力ファイルそのままの並び（`data.nodes`） |
| `--in <draft> --out <file>` | **並べ替えた後の並び**（`orderFlatNodes` の結果） |

契約は「**出力を `--check` へ戻したら、同じ位置を同じ言葉で言う**」で、`src/modules/logic-tree/skill-write.smoke.test.ts` の6ケース目がこれを縛る。

**計画は「並べ替え後の配列で検証する」と書いており**（後述の「計画自身の誤り」2）、着手前の走査でそれを「常に入力の並びで報告する」へ直した。**その直し方は `--check` では正しく、`--out` では逆だった**——最終レビューで実測して見つかっている（後述の「計画自身の誤り」9）。

### `canonical.ts` のコピー検査に「網羅の強制」を足し、それを1箇所に置いた
...
各モジュールの `skill-copy.test.ts` にある `canonical.ts` の行は**消していない**——あちらは「値 import を持たない」「消去できない構文が無い」の検査も一緒に回しているためで、消すとその検査まで落ちる。**したがって1箇所に寄せたのは網羅の強制だけで、バイト一致の検査そのものは各モジュールにも残っている**（同じバイト一致が2箇所で見られる形になるが、意図してそうしている）。
...
**全ブランチの最終レビューで見つかったもの（2件）:**

8. **計画が示した `grade.mjs` のコードが、実行ハーネスと噛み合わない独自形だった。** 計画本文は「`sequence-register/evals/grade.mjs` と**同じ骨格**」と指示していながら、**その直後に貼られたコードが骨格から外れていた**——run ディレクトリの走査（`ITER` 直下を全部見る／`eval-` で絞って `with_skill`・`without_skill` の層を見る）、プロジェクトの場所（`<runDir>/project`／run ディレクトリそのもの）、eval id の取り方、`grading.json` の項目名（`{run, passed, checks:[{name,ok,note}]}`／`{run_id, expectations:[{text,passed,evidence}], passed, total}`）の4点すべてが違う。**実ハーネスに掛ければ全5ケースが FAIL する**（`runDir` が `iteration-N/eval-0` なので `evalId` が `NaN` になり、`<runDir>/project` も存在しない）うえ、項目名が違うのでレビュー生成側が読めない。**Task 6 の実装もレビューも、指示文ではなく貼られたコードのほうに従っていた。** 判定内容は保ったまま兄弟3本の骨格へ寄せた
9. **着手前の走査による「計画の誤り2」の直し方が、半分だけ誤りだった。** 「整合性は**入力ファイルの並び**で報告する」という裁定は、**`--check` では正しいが `--out` では逆である**——`--out` のときアプリが開くのは**並べ替えた後の出力ファイル**なので、入力の並びで報告すると下書きの位置を答えてしまう。しかもこれは common path で、`SKILL.md`「7. 既存ファイルへの書き足し」は既存 JSON 全体を `--in` に渡させるため、下書きはたいてい DFS 順ではない。**smoke テストの既存5ケースは message の検証をすべて `--check` 経由で行っており、この欠陥を構造的に捕まえられなかった**——契約（出力を `--check` へ戻したら同じことを言う）を直接縛る6ケース目を足して塞いだ。**「片方の経路でしか成り立たない規則を、両方の経路に一律で適用した」形の誤りである**
...
- **4章（`canonical.ts` のコピー）**: 「登録**4** Skill」→**5 Skill**。**一致検査が3ファイルに散っている**という記述を、**logic-tree-m2 で「網羅の強制」を1箇所に置いた**へ全面的に書き替えた。あわせて「**`BUNDLED_SKILLS` から `map` で導出しないことが肝である**（導出すると恒真式になる）」を明記した——ここを外すと網羅を縛る意味が消えるため。**1箇所に寄せたのは網羅の強制だけで、バイト一致の検査は各モジュールにも意図して残っている**ことも添えた（最終レビューの指摘。当初は「1箇所へ集約した」とだけ書いており、バイト一致まで移したように読めた）
...
- **4章（配列順）**: 「**書き出しスクリプトが配列順を正規化するか**はコピーの可否で決まる」を子項目として新設した。sequence / issue-tree が触らないのは制約であって好みではないこと、コピー可能にできた時点で持たせるのが正であること、**ただし整合性と未記入の報告は「アプリが開くことになるファイル」の並びで行う**こと（`--check` は入力の並び、`--out` は並べ替えた後の並び）を書いた。**当初は「並べ替え前＝入力ファイルの並びで行う」と書いており、rev が誤った規則を正としていた**——最終レビューで直した（上の「計画自身の誤り」9）
