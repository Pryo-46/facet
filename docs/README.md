...
| UI の見た目がなぜそう決まったか（色は意味だけ・欠落は線・判断は面） | [`facet-UI設計ノート.md`](facet-UI設計ノート.md) — **UI の設計ノート**（診断と決定 D1〜D19。**A〜F への分割は設計スペック [`superpowers/plans/2026-08-23-m21-design-tokens-v2-design.md`](superpowers/plans/2026-08-23-m21-design-tokens-v2-design.md) の末尾にある**。A（色の規約）は M21、C（未定義表現の本体）は M22、B（タイポグラフィ）は M23 で実装した。**D（レイアウト固定）は M24〜M26 で決着した**——ノード幅の固定は M24 で実装し、**テーブルの行高固定＋2行省略は「変えない」と決め**、**ツリーの高さの打ち切りは実装したあと実機で一度撤回した**（`textarea` がキャレットに追従しない。D3 の「M24 での実施結果」の追記節）が、**2026-08-26 に人間が「高さは内容が決めるままを確定」と裁定して打ち切った**（D3 の「M26 での決着」の追記節）。**F（見送り集計の別枠・select 置換・角丸統一）は M25 で決着した**（U4「破線エッジの意味」も同時に決着）。**E（フォント同梱）は M26 で実装した**——IBM Plex 3書体を woff2 のみの生成 CSS で同梱し、D8 は「カラム名の階層をウェイトに移さない」という形で決着した（U2・U3 も同時に、実装からの言語化として書き起こした）。**A〜F すべて実装済みで、UI ノートの実施項目は全部閉じている**（§8）） |
...
| [issue-tree-m4](history/issue-tree-m4-schema-v3.md) | スキーマ v3——仮説を title/detail/value に割り、asks・feedbacks・resolved・date を足す | 課題ツリー・コア |
...
- `schemas/*.schema.json` — 各ツールのデータ形式の**正**。型（`src/types/*.ts`）はここから生成する。**同梱 Skill はこれのバイト一致コピーを持つ**——登録 Skill 5本すべてが `.claude/skills/<Skill名>/schemas/` に写しを同梱する（当初は「コピーを作らない」としていたが、sequence-m4・M15 で改めた。アプリが配布するのは `.claude/skills/` だけで、リポジトリの `schemas/` は出荷先に届かない）。**正が2つになる不安は、バイト一致をテストが強制することで引き受ける**（`src/core/skill-schema-copy.test.ts`。ズレた瞬間に赤くなる）。経緯は [`overview-rev.md`](overview-rev.md) 4章
