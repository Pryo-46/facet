...
- **`CellInput` の `autoSize` の JSDoc が「5行上限に切り詰められる」と書いているが、`MAX_ROWS` は 8**（`src/components/CellInput.tsx`）: M10 で5→8に変えた際の取り残しで、logic-tree M1 の計画本文が古い数値をそのまま引き継いだ結果、**誤った数値がソースにコミットされている。** 1語直すだけだが、放っておくと「この部品は5行まで」と信じたコードが書かれる `[logic-tree-m1]`
