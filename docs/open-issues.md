...
- **4本の登録 Skill の evals の判定器にテストが無い**（`plugins/facet/skills/{write-term,write-error,write-sequence,write-logic-tree}/evals/grade.mjs`）。判定器が壊れても evals の実走まで気づけない。`write-decision-table` は `src/modules/decision-table/skill-grade.test.ts` が合成した出力で検査している。
