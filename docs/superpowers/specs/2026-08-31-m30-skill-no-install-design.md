...
| 置かれるディスク | 数百 MB（`node_modules`） | 生成物 数十 KB / Skill（実測: validator 単体で 59,716 バイト） |
...
standalone の生成は `code: { source: true, esm: true }` を渡した Ajv2020 で行う。実測（`schemas/issue-tree.schema.json`）:

- 出力 **59,716 バイト**、残る `require()` **0件**
- お手本 `sample-project/課題ツリー.json` を `true`、`schemaVersion` を壊した版を `false`
- エラーオブジェクトの形は ajv 本体と同一（`instancePath` / `keyword` / `params` / `message`）
...
**削ってよい理由**：この検証の目的は「アプリが開けるファイルか」を保証することであり、その基準は同梱スキーマそのものである。外部のスキーマを渡して検証を通しても、アプリが開ける保証は増えない。むしろ**古いスキーマを渡して通してしまう**経路になる。

`schemas/<name>.schema.json` のコピーは**残す**。各 `SKILL.md` が「構造の詳細は `schemas/<name>.schema.json` を読む。スキーマが正」と読み手に指示しているためで、位置づけが**読み物として要る／検証の入力ではなくなる**へ変わる。`src/core/skill-schema-copy.test.ts` のバイト一致検査はそのまま。

エラー出力の `スキーマ: <path>` 行は、実体のパスを指せなくなるので落とす。
...
- **`docs/open-issues.md`**——実機確認が未実施の3項目（「次に手を付ける候補」の4・7・9番）が確認事項として「置かれた先で `npm install` した後の状態」を挙げている。「置かれた先で**追加の手順なしに**動くこと」へ書き換える
- **`docs/overview-rev.md`** 3箇所（**4章に2つ、5章に1つ**）
  ...
  - **5章** スキーマの実体を共有する項——検証の入力ではなく読み物になった旨
