...
| 11 | **フォルダ**をドロップする（ファイルではなく）→ どう見えるか。`fileReferences` は `@docs ` を素直に吐く | 気になるなら弾く。ここでは観察だけ |
| 12 | 終了済みのタブがアクティブな状態で `@` → **新しいタブが立って**差し込まれる | — |
| 13 | 起動中（約1秒）に `@` を押す → 差し込みが待ち行列に積まれ、**押された順**（`initialText` → `insertion`）で流れる | 実害が見えたら順序を入れ替える |
...
- **`readClipboardText` が「クリップボードが空」と「読み取りに失敗した」を区別せず、どちらも空文字に潰していること。**
  その結果 `TerminalTab` の貼り付け失敗の経路（`onError` → トースト）は**本番では到達不能**で、
  `clipboard-manager:allow-read-text` が外れると**右クリック貼り付けは無反応になり画面に何も出ない**。
  **無反応を見たらまず capabilities の権限を疑うこと。** 見分ける改修をしないのは、プラグインが両方で例外を投げ、
  メッセージ文字列を解析しないと区別できないため（非公開の文言への依存は新しい負債になる）

- [ ] **Step 6: `overview-rev.md` へ反映する**

- **4章**: 端末ペインへファイル参照を渡す動線があること。**渡すのは参照だけで、facet は
  ファイルの中身も会話も読まない**こと（既存の「AI はアプリに組み込まない」の例外記述の隣）
- **7章 ← 最終レビューで追加。落とすと嘘が残る。** 現在この章は太字で
  「**プラグインの読み取り権限（`clipboard-manager:allow-read-text`）は与えていない**」
  「**この例外はプラグインの読み取り権限を増やさない**——`clipboard-manager:allow-read-text` は今回も与えていない」
  と2箇所で言い切っている。**M28 はこれを与えた**（`src-tauri/capabilities/default.json`）。
  「**テキスト**の読み取りは端末の右クリック貼り付けのために与えた。**HTML** の読み取りだけは今も与えず、
  自前コマンド `read_clipboard_html` を通す」へ直すこと。
  この章は当初の計画の反映先リストから**漏れていた**——`CLAUDE.md` が M4 の教訓として名指しする
  「反映漏れは設計と実装の食い違いとして伝播する」そのものなので、**必ず直す**
...
- [ ] **Step 6b: 設計文書を訂正する（最終レビューで追加）**

設計文書（`2026-08-29-m28-claude-pane-handoff-design.md`）は「正」ではなく記録だが、次のマイルストーンの
計画者が §6 を読んで同じ穴に落ちるので直す。

- **§6.2**: `App.tsx` から `getCurrentWebview()` を直接呼ぶ記述になっているが、**実装は
  `src/fs/drag-drop.ts` を経由する**。理由は2つ——(a) このリポジトリは Tauri を触るコードを
  `src/fs/*` に置く規則で、`App.tsx` 直呼びはそれを破る (b) `getCurrentWebview()` は Tauri の外
  （jsdom）で**同期 throw** し、`.catch()` では捕まらないので既存の App テスト49本が落ちる。
  `async` 関数で包むと同期 throw が拒否 Promise になり `.catch()` が効く
- **§6.4**: 実際に出荷した挙動を書く——**ドロップはペインが見えているときだけ効く**（畳むと
  `display:none` で矩形が 0 になり必ず「外」）。一方**`@` ボタンは畳んでいても効く**。
  トリガーによって規則が違うことを、次の計画者がコードから発見し直さなくてよいように残す
- **§5.2**: 手順2の「タブが**無ければ**」は、正しくは「**実行中のタブが無ければ**」だった。
  終了済みのタブは台帳に残り `activeId` も指したままなので、この書き方のままだと死んだ PTY へ
  書きに行く（最終レビューで発見し `351ce0e` で修正済み）
- **§2.1**: bracketed paste の但し書きを足す。xterm が `ESC[200~`/`ESC[201~` で囲むのは
  **アプリが DECSET 2004 を送った後だけ**（`node_modules/@xterm/xterm/lib/xterm.js` を実読して確認）。
  **起動時の差し込み（トリガー1）は claude の TUI が立ち上がる前なので囲まれない。** つまり
  「`@` の補完に食われる懸念が減る」という §2.1 の効能は、4つのトリガーのうち起動時だけ効かない。
  §4.3 の「最初の出力から N ms 静穏」への差し替えは、**消える問題と補完の問題の両方**に効く手当てになる
