# ロジックツリー M3 設計: Miro のマインドマップとクリップボードで交換する

2026-08-29 / logic-tree-m3
...
| ボタン | 活性の条件 |
| --- | --- |
| Markdown 系 | ファイルを開いていて、そのツールが `outputs` を持つ（ロジックツリーは M3 から活性） |
...
ロジックツリーは `outputs: []` のままだった（M1 の「M2 で足す」が logic-tree-m3 に持ち越された）。
