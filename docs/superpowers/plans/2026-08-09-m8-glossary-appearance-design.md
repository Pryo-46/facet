...
- 利用可能幅は `pointerdown` の時点で親の `clientWidth` を1度読めば足りる（ドラッグ中に窓は変わらない）
- **窓を狭めたときは定義列が縮んで吸収する。横スクロールは出ない。** 定義列が残りを取る構造（決定1）の必然的な帰結である——テーブルは常に親幅なので、`overflow-x` に受けさせる余地がそもそも無い。狭すぎると感じたら他の列をドラッグで狭める、が操作としての答えになる
- **ダブルクリックで既定幅に戻す**
...
検証するのは `{ink, ink-muted} × {warning/20, warning/10} × {canvas, surface} × {ライト, ダーク}` の16通り。背景に `canvas` を含めるのは、テーブルの面が `surface` である以上いまは `surface` だけで足りるが、M7 の教訓「背景は `canvas` と `surface` の両方を見る」を守るコストがループ1本ぶんしかないため。

**`text-warning` はこれらの面の上に置かない**（決定12）ので、組み合わせに含めない。

### 決定12: プレースホルダは `text-ink-muted` にする

`GlossaryEditor.tsx:369` の `placeholder:text-warning/70` は、決定11の合成モデルで測ると **2.80:1**（`open-issues.md` が実測として記録した「約 2.8:1」と一致する。モデルが正しいことの裏付けでもある）。

`/70` を外して `text-warning` の不透明にする案は**成立しない**。測ると `warning/10` の面の上で **4.59:1** で、4.5 は超えるが M7 が定めた3%の余裕（4.635:1）に届かない。素の `surface` の上ですら 5.28:1 しかないため、面に色が付いた時点で余地がない。

**`text-ink-muted` にする。** 役割トークンの定義がそのまま「抑えた文字」であり、プレースホルダはまさにそれである。「未定義を負債として可視化する」（rev 5章）は**セルの面が warning 色を持つことで担われている**ので、文字色まで warning にする必要はない。むしろ同系色の上に同系色を置くことになり、WCAG の数値以前に読みにくい。

### 決定13: `errorCell` = `bg-warning/20`、`warnCell` = `bg-warning/10`

決定11の合成モデルで全組み合わせを測って確定した。要件は本文 4.5:1、M7 の教訓に従い3%の余裕（4.635:1）を課す。

| 面 | 背景 | `text-ink` | `text-ink-muted` |
| --- | --- | --- | --- |
| `warning/20` | ライト `surface` | 12.04 | 6.68 |
| `warning/20` | ダーク `surface` | 8.44 | **5.01** |
| `warning/10` | ライト `surface` | 13.93 | 7.72 |
| `warning/10` | ダーク `surface` | 9.94 | 5.90 |

最も厳しいのはダークの `surface` 上の `ink-muted` で 5.01:1。**`/25` は同じ条件で 4.58:1 に落ちるため使えない**——これが現行値を変える理由である。濃さが 2 倍あるので `error > warn` の強度差も保たれる。

`text-warning` はこの2つの面の上に置かない（決定12）。したがって縦帯への切り替え（当初の予備案）は不要になった。波線下線が表記ゆれの「指摘（suggestion）」用に予約されている（session-notes 論点5、rev 9章）ことは変わらない。
