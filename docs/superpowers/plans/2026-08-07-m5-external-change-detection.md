...
4. **取り込みは `selectFile` に合流させる。**——**M3 の申し送りは「取り込みは `applyEdit` の4本目の経路になる」と予告していたが、そのままでは成立しない。** `applyEdit` は「自動保存へ渡す」＋「履歴に record」だが、取り込みはディスクを正として**履歴を作り直す**（破棄する）操作であり、自動保存へ押し戻す必要もない。M1 で確定した「ファイル選択時は必ずディスクから読み直す」経路（`selectFile`）が、読み直し・`computeIssues`・saver の張り直し・`createHistory` を1本で揃える。**`applyEdit` を通るのは二択ダイアログの「自分の編集で上書き」側**（これが4本目）。
...
6. **`flush()` は chain が静止するまで繰り返す（上限5回）。削除経路には `settle()` を新設する。**（静止の判定は `chain === target` だけで行い、`pending === null` は戻り値の意味として分ける。混ぜると失敗した write を同一 flush 内で5回書き直し、M4 の「再試行は次の flush」意味論を壊す——実装時に確認済み。） 10節が挙げた2つの直し方のうち②を採る。①（削除確定時に入力を切る）も Task 9 で入れる——②だけでは「待っている間に打鍵できる」構造自体は残るため。**`trashFile` の待ちは `flush()` から `settle()` へ移す**：ループ化した `flush()` は、失敗して復元された pending を書き直してしまい、「消したファイルを書き戻さない」という `trashFile` の保証を壊す。`settle()` は「進行中の write の完了を待つが、何も書かない」。
...
- Produces:
  - `AutoSaver.flush(): Promise<boolean>` — 意味論を変更。**chain が静止するまで**最大 `FLUSH_MAX_ROUNDS` 回繰り返し、静止時点の書き残しの有無を返す（true＝書き残しなし）。失敗した書き込みの再試行はこの呼び出しの中では行わない（次の flush／タイマーに任せる。M4 の意味論のまま）
...
export interface AutoSaver {
   ...
   * 解決してしまう（進行中の write を残したまま close のゲートを通る。M4 の申し送り）。
...
`return { ... }` の `flush` を置き換え、`settle` と `hasUnsaved` を足す。**`chain === target`（静止したか）と `pending === null`（書き残しがあるか）は別の判定であり、1つの式に混ぜてはいけない**——混ぜると「失敗して復元された pending」がループの継続条件になり、同一 flush 内で最大5回の書き直しが起きて、M4 が確定した「再試行は次の flush／タイマー」という意味論を変えてしまう（実装時に実際に踏み、既存テスト『onError は1回』が壊れて判明した）:
...
**M4 の申し送りが「データ喪失。M5 で必ず塞ぐ」と指定した項目。** 現状 `createFile` は衝突回避の名前を走査時スナップショット（`existingNames`）だけから決めるが、`writeTextFile` は既存ファイルを切り詰めて書く。踏む手順は本プロジェクト自身のワークフローそのもの（空フォルダを開く → Skill が `用語集.json` を書く → アプリの空状態から「用語集を作る」→ Skill の用語集が空の用語集に置き換わる）。確認もエラーも出ない。
...
M4 の申し送りが「M5 が3人目の生産者を連れてくるので、この時点で queue 化かスロット分離を判断すること」と指定した項目。**キューにする**（設計決定9）。このタスクは**既存の挙動を保ったままの移行**で、外部変更の生産者は Task 8 で足す。
...
> ここで落ちる可能性がある点: `AlertDialogTitle` は h2 として描画されるので `getByRole('heading')` で引ける（`ConfirmDialog.dom.test.tsx` は `getByText` で引いており、M4 の申し送りが「小さな負債」として挙げている形）。もし role で引けなければ**ライブラリの実装を読んで確認し、テスト側を実態に合わせる**（計画の記述より実装が正）。
...
```ts
  ...
  //「外部変更の二択」の3つ（M4 の申し送り。スロット1つでは要求が無言で落ちる）。
...
`ensureGlossary` の冒頭を差し替える（**M4 の申し送りのデータ喪失の再現手順そのものを塞ぐ**——空フォルダを開いた後に Skill が用語集を書いた状態で押されるボタンなので、押下時点のスナップショットは必ず古い）:
...
`createNewFile` は再走査しない（名前解決がディスクを見るので上書きは起きず、singleton でないモジュールは2つ作れて構わない）。**代わりに `addCreatedFile` に同一パスの二重登録ガードを入れる**（M4 の申し送りの残件。ダブルクリックや遅い IPC で同じパスのエントリが2件でき、`key={file.path}` が重複し単一性違反バッジまで出る）:
...
M4 の申し送りの残件。取り違えると interceptor ループ（**閉じられなくなる**）が再発する。
...
M4 の申し送りの残件（「脱出口として最悪の失敗の仕方をする」）。`showModal` の `onConfirm` は `void` で起動されるので、`forceClose()` の reject は誰も受け取らない——ダイアログは閉じ、`dispose()` が保留編集を捨てた後なので、ユーザーには「押したのに何も起きない（そして編集は失われた）」としか見えない。
...
M4 の申し送りが挙げた2つの直し方の①（②は Task 1 の `flush` ループ化で入れた）。`trashFile` が in-flight の write を待つようになった結果、**待っている間もエディタは同じ saver を掴んだままマウントされている**という残余の窓がある。`trash()` の**前**に切り離す。
...
```ts
   ...
   * 生きた write を残せる（M4 の申し送りの残余の窓）。選択と saver を先に
...
```ts
  ...
  const closeCurrentFile = async (): Promise<boolean> => {
    ...
    //  切り替えたときに前のファイルのバナーが残る。M2 の申し送り）
    setSaveError(null)
    return true
  }
```
...
M3 の申し送りの「別名パネルを開いたまま外部変更が来ても表示は追随する**はず**」を、テストで「する」に変える。`AliasCell` の下書き再同期は `aliases` の**内容**比較なので追随するはずだが、外部変更は Undo/Redo と違って**データが丸ごと別物に差し替わる**経路であり、既存の回帰テスト（履歴を跨ぐ参照比較の件）とは入力が違う。エディタは `key={selected.path}` なので、同じパスへの取り込みでは**再マウントされない**——ローカル下書きが残ったまま親の値が変わる、まさに事故が起きる形。
...
**準備:** worktree で確認するなら、先に `Get-NetTCPConnection -LocalPort 5173` で他のチェックアウトの dev サーバーがポートを掴んでいないことを確認する（掴まれていると古いコードのアプリが表示される）。

**確認用フォルダは、リポジトリの外に新しく作った空フォルダを使うこと**（例: `C:\Dev\facet-verify`）。**`sample-project/` を使ってはいけない**——あの `glossary.json` は `src/core/canonical.test.ts` の「Skill が書いたファイルを読み→書き出してバイト一致」という最重要の回帰テストの fixture であり、削除したり中身を差し替えたりすると `npm test` が落ちる（M5 の実機確認で実際に2回落とした）。`git diff` / `git restore` を使いたい確認（②の退避の検証）だけは、そのために**別の git リポジトリを1つ作る**か、確認後に `git restore sample-project/glossary.json` で必ず戻すこと。

**外部からの書き換え・作成は、Claude に依頼するのが確実**（「そのフォルダに用語集を書いて」）。エクスプローラで貼り付ける方法は、**貼り付け先を間違えても何も起きないだけなので気付けない**（M5 の実機確認で、アプリが監視していないフォルダに貼って「検知されない」と誤判定した）。どのフォルダを監視しているかは、監視開始時のログか、アプリのヘッダに出ているパスで確認する。
...
- [ ] **5. 外部でファイルが増える／単一性違反（完了条件7）**: 用語集があるフォルダへ、外部から2つ目の用語集 JSON をコピー → 一覧に増え、両方に単一性違反の赤バッジが出る → 片方を削除 → バッジが消える。**M4 の完了条件3はアプリ内だけでは再現できなくなった**（M4 の申し送り）が、監視が入ったこの経路で確認できる。
- [ ] **6. Skill との同居（完了条件9。M4 の申し送りのデータ喪失の再現手順そのもの）**: 空フォルダをアプリで開く（「このプロジェクトにはまだ用語集がありません」が出る）→ アプリはそのまま触らず、Claude に `glossary-term-register` で用語登録を頼む → **監視でトーストが出て一覧に用語集が現れ、「用語集を作る」ボタンが消えること**。もしボタンが残っていたら押してみて、**Skill の用語集が上書きされず、2つ目（`用語集-2.json`）が作られること**を確認する（上書きされたら Task 2 が効いていない）。
...
**結果の記録:** 見つかった不整合は、修正するかどうかに関わらず**実装スコープ定義書の「M5 完了に伴う申し送り」に書く**——レビューの記録は作業ワークスペースと一緒に消えるため、そこが唯一の恒久的な記録である（M4 の申し送り）。
