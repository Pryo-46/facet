...
> **この節は着手後に直した。** 当初は `line-height: 1.2` を直接書く指示だったが、生成 CSS が `.text-sm{line-height:var(--tw-leading,1.65)}` であり、**クラスセレクタが要素セレクタに勝つため実質すべてのボタンで無効**だった（レビューが Tailwind の実コンパイラを呼んで検出）。`npm test` / `tsc` / `lint` はいずれもこの種の欠陥を捕まえられない。**実装後に `npx vite build` で生成 CSS を確認すること。**
...
- [ ] **4. 折り返しと5行上限** — **まず「2行目が出るか」を見る。** 高さ計算は `getComputedStyle` の `line-height` を px として読み、読めなければ安全側に倒して1行のままにする。つまり**折り返しが丸ごと効かない失敗モードは無言**であり、jsdom でも捕まらない。2行目が出ることを確認したうえで、長い定義を入れて5行で止まりセル内スクロールになるかを見る。窓を狭めたとき定義列が縮んで吸収するかも確認する
...
**最終レビューが継ぎ目で見つけた3件の回帰確認（11〜14）。** いずれもコードでは直したが、実機でしか最終確認できない。

- [ ] **11. 別名パネルを「最終行」で開く** — パネルが下端で切れずに全部見えるか。**別名列と備考列を両方いちばん狭くした状態でも**右端で切れないか（テーブルを包む div から `overflow-hidden` を外して直角にした。角丸が無いことの見え方もここで判断する）
- [ ] **12. 列幅を変えた後に折り返しが追随するか** — 長めの定義を入れた状態で定義列・備考列をドラッグで狭め、**隠れる行が出ないこと**。窓リサイズでも同じ（`ResizeObserver` を張った）
- [ ] **13. フォーカス位置が見えるか**（ライト・ダーク両方） — セル間を Tab で移動して、いまどのセルにいるか分かるか。**エラーセル・未定義セルにフォーカスしたとき、警告色が消えずにリングと共存するか。** 別名チップがチップに見えるか
- [ ] **14. 窓幅を 1000px 未満へ縮める** — 固定4列の合計は 736px で、サイドバー 256px と余白を足すと**窓幅 1030px 付近で備考列が切れ始める**。横スクロールは出ない設計なので、切れたときに操作不能にならないか（備考列のハンドル自体が画面外に出るため、狭める操作は名称・種別・別名のハンドルからしかできない）。会議室の 1024×768 投影で現実に起きる条件

- [ ] **15. 後片付け** — `git checkout -- sample-project/ && git clean -fd sample-project/` を実行し、`git status --short` が空になること
