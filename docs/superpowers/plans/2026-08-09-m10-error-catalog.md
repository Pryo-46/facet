...
5. **「関与しないレベルの空欄」の3点セットを一度に裁定する**（最終レビューが挙げた設計の食い違い）。`resolutionLevel: user` のエラーを1件入れて、次の3つを**同時に**見る:
   - 画面のセルの**黄色**: 「サポートの対応」「エンジニアの対応」は黄色くならない（決定14。関与しないレベルは信号を出さない）
   - 画面の**プレースホルダ**: いまは同じセルに「未定義」と出る（`notes` 以外の空欄すべてに出す実装）
   - **サポート向け Markdown**: いまは同じセルが `（未定義）` になる（決定15。空フィールドは `（未定義）`）

   **決定14（画面の黄色）と決定15（出力の未定義）は、関与しないレベルについて食い違っている。** 決定14 の論拠（「ほとんどのエラーは1レベルしか関与しないので、全部を警告にすると信号が死ぬ」）は出力にも当てはまるが、決定15 には例外規定が無い。その結果、**サポート向け出力は行の大半が `（未定義）` を2つ抱える**（`user` レベルなら「サポートの対応」「エンジニアの対応」の2列）。

   実機で3つを並べて見て、次のどれかを選ぶ:
   - **(a) 現状のまま**（未定義は消さない、が徹底される。ただし出力は `（未定義）` だらけになる）
   - **(b) 画面のプレースホルダだけ warning と揃える**（`placeholder={isWarnCell(entry, field) ? '未定義' : undefined}` の1行。出力は現状のまま）
   - **(c) 画面と出力の両方を warning の判定と揃える**（出力側も「関与しないレベルの空の Action」は空欄で出す）

   **片方だけ直すと画面と出力がずれる**ので、3つを1つの判断として決めること。
6. **赤の指摘**: エラー名を2件同じにすると赤くなり、issue 一覧に出ること。`user` を宣言して「ユーザーの対応」を空にすると、そのセルが赤くなること
7. **行数上限（決定17）**: 対応文に長文（100文字程度）を入れ、8行まで伸びてから内部スクロールになること。**初回マウントの体感**（行数ぶんの強制リフロー。`open-issues.md` 性能の項）が悪化していないか
8. **プロファイル切替**: サポート向け⇄開発向けで列が増減すること。**横スクロールが出ないこと**。各プロファイルの列幅が独立して保持されること
9. **書き出し**: 額縁のボタンがドロップダウンになり、「サポート向け」「開発向け」を選べること。書き出したファイル名が `エラーカタログ-サポート向け.md` / `エラーカタログ-開発向け.md` になること。コピーも同様に2択になること
10. **用語集が変わっていないこと**: 用語集を開き、ボタンが2つのまま（ドロップダウンになっていない）で、書き出し名が `<用語集のファイル名>.md` のままであること
11. **単一性違反**: エラーカタログをもう1つ作ろうとするとボタンが disabled であること。外部で2つ目を置いた状態でフォルダを開き直すと、単一性違反が表示されること
12. **外部変更の取り込み**: エディタで開いたまま外部エディタで `errors` を書き換え、取り込みが効くこと
