...
`sample-project/` は **README から参照するお手本であり、同時に動作確認の遊び場でもある**。JSON 5本は追跡対象なので、実機確認で編集したら元に戻す（お手本の変更は、意図してそう決めたときだけコミットする）。`.claude/` は旧版が置いた残骸、書き出した `.md` は確認の痕跡で、いずれも `sample-project/.gitignore` で追跡外にしてある。
...
`-x` を付けるのは、`.gitignore` した `.claude/`（旧版が置いた残骸）と `.md`（動作確認で書き出したもの）も落とすため。**お手本の JSON は追跡対象なので `checkout` が戻す。** `clean` では消えない。
...
- コメントは「いまの値・構造の理由」と「踏むと壊れる罠」を現在形で書く。マイルストーン番号・レビュー指摘・依頼者の指示・変更前の状態・「〜で確定した」の記録は書かない
