export function poseQuestions(step) {
    switch (step.kind) {
        case 'self':
 ...
 */
export function presentAnswers(step) {
    ...
    if (f.failed !== undefined)
        present.push('failed');
    if (f.unknown?.decision !== undefined)
        present.push('unknown');
    if (f.unknown?.ifExecuted !== undefined)
        present.push('ifExecuted');
    return present;
}
/** 在るのに問いが立っていない答え（種別切替の残骸）。ガターのグレースロットと整合性検証が使う */
export function unposedAnswers(step) {
 ...
 */
export function questionLabels(step) {
    if (step.kind === 'self') {
 ...
 */
export function questionHints(step) {
    if (step.kind === 'self') {
        return {
 ...
 */
export function readSlot(step, path) {
    if (path === 'failed')
        return step.failures?.failed ?? {};
    if (path === 'unknown') {
