 ...
 */
export function productSize(conditions) {
 ...
 */
export function valueIndicesAt(conditions, position) {
    const out = [];
    let rest = position;
...
export function identityAxes(conditions) {
 ...
 */
export function axesById(prev, next) {
    return next.map((c) => {
...
export function outcomeFromById(prev, next) {
    return next.map((o) => {
 ...
 */
function sourceRows(prevConditions, prevRows, axes, indices) {
    const pin = prevConditions.map(() => null);
 ...
 */
export function rebuildRows(prevConditions, prevRows, nextConditions, axes, outcomeFrom) {
    ...
    for (const prev of prevRows) {
        for (const value of prev.results)
            if (value !== '')
                filled += 1;
    }
 ...
 */
export function alignRowsByLabel(conditions, outcomeCount, rows) {
    ...
    for (let position = 0; position < total; position++) {
        ...
        const same = (row) => row.impossible === first.impossible && row.results.every((v, j) => v === first.results[j]);
        if (!group.every((i) => same(rows[i])))
 ...
 */
export function rebaseRows(base, next) {
    ...
    const built = rebuildRows(base.conditions, prevRows, next.conditions, axesByDefinition(base.conditions, next.conditions), outcomeFrom);
    return { ...built, lostCells: built.lostCells + choiceLost };
}
/** 値の対応づけ。本数が同じなら位置、違えば一意に引けるラベル */
function axesByDefinition(prev, next) {
    return next.map((c) => {
        const from = prev.findIndex((p) => p.id === c.id);
        if (from < 0)
            return { from: null, valueFrom: [] };
        const old = prev[from].values;
        if (old.length === c.values.length)
            return { from, valueFrom: old.map((_, v) => v) };
        return {
            from,
...
function choiceMap(prev, next) {
    const map = new Map();
    prev.forEach((label, k) => {
        if (map.has(label))
            return;
        if (prev.length === next.length)
            map.set(label, next[k]);
        else
            map.set(label, next.includes(label) ? label : '');
    });
    return map;
}
