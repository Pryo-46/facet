 ...
 * 永遠に埋まらない欠落が残る
 */
export function isMissingResult(row, outcomeIndex) {
    if (row.impossible)
        return false;
 ...
 * 数える箇所が同じ関数から出る（規約4）
 */
export function tallyMissing(data) {
    let result = 0;
    for (const row of data.rows) {
        for (let j = 0; j < data.outcomes.length; j++) {
            if (isMissingResult(row, j))
                result += 1;
        }
    }
    let label = 0;
    for (const c of data.conditions) {
        if (isMissingLabel(c.name))
            label += 1;
    ...
    const parts = [
        { kind: RESULT_KIND, label: '未記入', count: result, variant: 'open' },
        { kind: LABEL_KIND, label: '名前なし', count: label, variant: 'open' },
    ].filter((p) => p.count > 0);
