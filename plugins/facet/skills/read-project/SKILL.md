...
- 立つ問いは4つで、いずれも導出される: **子を持たない課題**に仮説が0件なら「仮説なし」／仮説の `events` が0件なら「未決」／最新が `onHold` なら「保留」／**文言のある問い（`asks`）に FB が1件も無ければ「FB待ち」**
- **課題ノードの `events` に付く旗は `deferred`（見送り）と `resolved`（解決）の2種。意味は逆だが、どちらもその課題と配下すべての問いを止める。同時には立たない。** 抑制は祖先を遡って計算されるので、**子に旗をコピーしない**（親の旗を外したとき子が取り残される）
- **課題ノードの `events` だけは追記専用ではない。** 旗はアプリでは入り切りのトグルで、戻すと**最新の1件が消える**（解除を表す種別は無い）。だから**この列は 0 件か 1 件**で書く
- **`feedbacks` は記録であって下書きではない。** レビュー中の発言などが入り、**移動も昇格もせずその場に残り続ける**。判断の理由（`events[].note`）は**人が自分の言葉で書く**——FB の文言をそのまま `events[].note` へ写さない
- `detail` / `value`（仮説の作り方の詳細・価値仮説）や `asks`（聞きたいこと）が空なのは未決ではない。**欠落は仕様の穴ではない**ので、埋めるよう促さない
...
このフォルダの JSON を直接手で編集しない。種類に応じて `facet:write-term` / `facet:write-error` / `facet:write-sequence` / `facet:write-issue-tree` / `facet:write-logic-tree` / `facet:write-decision-table` を必ず使う（ID 採番・スキーマ検証・正規形書き出しを通すため）。対応する Skill が無い種類のファイルを編集する場合も、最低限次を守る: (1) ID は既存と同じ形式で、ランダムに振る（連番にしない） (2) 既存のキー順・インデント（半角スペース2）・末尾改行を保つ。
